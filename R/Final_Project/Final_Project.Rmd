...
**2) Load in Data and Change Titles**
  ...
  In this section I loaded in my data set. Then I had to change the column titles to match the original data set that originlly had fewer cycles listed out.
...
Models for all cycles were ddefined using the following variables. In the cooresponding summary tables, the number that follows the Model Letter corresponds to the cycle for which those models were run.
...
I | LarvaeAge | In Vitro Exsheatment + In Vivo Exsheathment | cycle_#NA_vitro
J | LarvaeAge | In Vitro Exsheatment + In Vivo Exsheathment + In Vitro Viability + In Vivo Viability + Hatchability| cycle_#NA_hatch
...
model_K <- lm(Cycle ~ InVitroViability + InVitroExsheathment + InVivoViability + InVivoExsheathment, data = no_hatch_col)
summary(model_K)
model_L <- lm(LarvaeAge + Cycle ~ InVitroViability + InVitroExsheathment + InVivoViability + InVivoExsheathment, data = no_hatch_col)
summary(model_L)
...
summary(model_O)
model_P <- lm(LarvaeAge + Cycle ~ InVitroViability + InVitroExsheathment + InVivoViability + InVivoExsheathment + Hatchability, data = marissa_data)
summary(model_P)
```

**Intercycle Modeling Statistics**
```{r Intercycle table, results='asis', message=FALSE, echo=FALSE, cache=TRUE}
stargazer(model_K, model_L, model_M, model_N, model_O, model_P, type = "html", notes.append = FALSE, notes.label = "Symbols", notes = c("<sup>&#42;</sup>p<0.1; <sup>&#42;&#42;</sup>p<0.05; <sup>&#42;&#42;&#42;</sup>p<0.01"), keep.stat = "adj.rsq", column.labels = c("K","L","M","N","O","P"), multicolumn = TRUE, ci = TRUE)
```
